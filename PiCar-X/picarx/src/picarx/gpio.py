...
class Pin(FileSystemEventHandler):
    ...
    def __init__(self, pin_number: Union[int, str], direction=Direction.OUT, edge=Edge.NONE, active=Active.ON, callback = None, emulator=False, loop=None):
    ...
    def __export(self):
        if not self.__emulator:
            ...
            print("Exported GPIO{}".format(self.pin_number))
        else:
            print("Did not export the pin {} since I am an emulator".format(self.pin_number))

    def __unexport(self, *args):
        if not self.__emulator:
            with open(Pin.SYSFS_GPIO_UNEXPORT, "w") as unexport:
                unexport.write('%d' % self.pin_number)
            print("Unexported GPIO{}".format(self.pin_number))
        else:
            print("I am an emulator, thus I will not unexport pin {}".format(self.pin_number))
...
class GPIO(object):
    ...
    def setup(cls, pin_number, direction, callback=None, emulator=False):
        if not Pin.exists_already(pin_number):
