...
class DCMotorEmulator(AbstractMotorEmulator):
    ...
    def __init__(self, name: str, direction_pin: Union[int, str], pwm_pin: Union[int, str], i2c_port: str, motor_side: MotorSide = MotorSide.LEFT):
        super(DCMotorEmulator, self).__init__(name, int(direction_pin), pwm_pin, i2c_port, MotorSide(int(motor_side)))
    ...
    def controller_publisher(self, publisher: rospy.Publisher):
        if publisher is None:
            ...
            rospy.logerr(publisher.name)
            if isinstance(publisher, rospy.Publisher):
                self.__joint_publisher = publisher
            else:
                raise TypeError("The Method joint_publisher expects a ROS Publisher object not the type: {}".format(type(publisher)))
    ...
    def drive_with_speed(self, i2c_value: int):
        ...
        self.controller_publisher.publish(Float64(self.direction * percentage))

    def start(self):
        rospy.init_node(self.name, anonymous=True)
        frequency = rospy.Rate(50)
        
        self.controller_publisher = rospy.Publisher(rospy.get_param("~controller_publisher_topic"), Float64, queue_size=5)
...
if __name__ == '__main__':
    ...
    dcmotor_emulator = DCMotorEmulator(options.args.name, options.args.direction_pin, options.args.pwm_pin, options.args.i2c_port, options.args.motor_side)
